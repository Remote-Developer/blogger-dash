  ...
  return (
    <>
      {(data as any)?.map((sdata: any, index: any) => (
        <div
          key={sdata._id}
          className="max-w-sm mt-4 p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700"
        >
          <div
            onClick={() => handleDelete((sdata as any)._id)}
            className="float-right text-red-700 cursor-pointer"
          >
            <RiDeleteBin5Line />
          </div>
